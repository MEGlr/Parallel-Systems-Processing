...
int main(int argc, char ** argv) {
        ...
        //datatype to receive columns ghost cells (east & west)
        MPI_Datatype col;
        ...
        int vertical=1, horizontal=0;
        MPI_Cart_shift(CART_COMM,vertical,1,&west,&east);
        MPI_Cart_shift(CART_COMM,horizontal,1,&north,&south);
        ...
        MPI_Request request_handlers_initial[6];
        MPI_Status status_initial[6];

        MPI_Request request_handlers_final[2];
        MPI_Status status_final[2];

        int initial_request_handlers_count = 0;
        int final_request_handlers_count = 0;
...
#define T 256
                for (t=0;t<T;t++) {
                        ...
                        //*************TODO*******************//
                        initial_request_handlers_count = 0;
                        final_request_handlers_count = 0;
                        ...
                        if (north != MPI_PROC_NULL){
                                MPI_Isend(&u_previous[1][1], local[1], MPI_DOUBLE, north, 0,
                                                MPI_COMM_WORLD, &request_handlers_initial[initial_request_handlers_count]);
                                initial_request_handlers_count += 1;
                        ...
                        if (west != MPI_PROC_NULL){
                                MPI_Isend(&u_previous[1][1], 1, col, west, 0, MPI_COMM_WORLD, &request_handlers_initial[initial_request_handlers_count]);
                                initial_request_handlers_count += 1;
                                MPI_Irecv(&u_current[1][0], 1, col, west, 0, MPI_COMM_WORLD, &request_handlers_initial[initial_request_handlers_count]);
                                initial_request_handlers_count += 1;
                        }
                        ...
                        if (south != MPI_PROC_NULL){
                                MPI_Irecv(&u_previous[local[0]+1][1], local[1], MPI_DOUBLE, south, 0, MPI_COMM_WORLD, &request_handlers_initial[initial_request_handlers_count]);
                                initial_request_handlers_count++;
                        }
                        // Communicate with west
                        if (east != MPI_PROC_NULL){
                                MPI_Irecv(&u_previous[1][local[1]+1], 1, col, east, 0, MPI_COMM_WORLD, &request_handlers_initial[initial_request_handlers_count]);
                                initial_request_handlers_count++;
                        }

                        MPI_Waitall(initial_request_handlers_count,request_handlers_initial, status_initial);
                        ...
                        if (south != MPI_PROC_NULL){
                                MPI_Isend(&u_current[local[0]][1], local[1], MPI_DOUBLE, south, 0,
                                                MPI_COMM_WORLD, &request_handlers_final[final_request_handlers_count]);
                                final_request_handlers_count++;
                        }
                        /* *SEND* data to east */
                        if (east != MPI_PROC_NULL){
                                MPI_Isend(&u_current[1][local[1]], 1, col, east, 0, MPI_COMM_WORLD,
                                                &request_handlers_final[final_request_handlers_count]);
                                final_request_handlers_count++;
                        }
                        MPI_Waitall(final_request_handlers_count, request_handlers_final, status_final);
#ifdef TEST_CONV
                        if (t%C==0) {
                                //*************TODO**************//
                                /*Test convergence*/
                ...
                if (rank==0) {
                        ...
                        //sprintf(s,"resGaussSeidelSOR_MPI_%dx%d_%dx%d",global[0],global[1],grid[0],grid[1]);
